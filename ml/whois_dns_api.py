...
def domain_info():
    ...
    try:
        ...
        deterministic_risk, deterministic_flags, page_analysis = apply_deterministic_rules(url, domain)
        ...
        page_analysis = page_analysis if isinstance(page_analysis, dict) else {}
        page_behavior = {
            'has_login_form': page_analysis.get('login_form_detected', len(login_flags) > 0),
            'login_forms_detected': page_analysis.get('login_form_count', len(login_flags)),
            'html_findings_count': len(browser_findings),
            'interaction_ready': True,
            'js_rendered_analysis': True,
            'playwright_used': page_analysis.get('playwright_used', False),
            'interaction_steps': page_analysis.get('interaction_steps', []),
            'rendered_html_excerpt': page_analysis.get('rendered_html_excerpt'),
        }
